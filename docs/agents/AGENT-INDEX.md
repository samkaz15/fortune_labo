...
Which agents exist as specifications in this repository, as opposed to being
listed as intended above.
...
in that agent's own directory rather than worked around silently — see
`fortune_labo/agents/sns/x/workflow.md` (Interim operation) for the current
example.

## Channel sub-agents

A10 is implemented as a parent scope with channel sub-agents rather than a single
multi-platform agent, because the channels differ enough that one shared
